...
1\. \*\*Multi-Architecture\*\* : Compatible avec toutes les architectures (x86, x86\_64, ARM, anciennes machines)
...
├── create\_image.py         # Script Python d'assemblage de l'image disque
...
├── run\_x86\_64.bat          # Lance QEMU 64-bit (Windows)

├── run\_x86\_32.bat          # Lance QEMU 32-bit (Windows)
...
3\. \*\*Stockage des informations\*\* dans `INFO\_BLOCK` (0x0500)
...
&#x20;   mov  \[INFO\_BLOCK + 0], ebx    ; "Genu"

&#x20;   mov  \[INFO\_BLOCK + 4], edx    ; "ineI"

&#x20;   mov  \[INFO\_BLOCK + 8], ecx    ; "ntel"
...
&#x20;   mov  byte \[INFO\_BLOCK + 0x0C], 1    ; 64-bit supporté
...
&#x20;  - Convertit en MB et stocke dans `INFO\_BLOCK+0x0E`
...
&#x20;   mov  \[E820\_MAP - 2], cx      ; stocke nombre d'entrées à 0x1FFE
...
&#x20;   ; Vendor (déjà stocké dans INFO\_BLOCK par Stage 0)
...
&#x20;   movzx ecx, word \[E820\_MAP - 2]   ; nombre d'entrées
...
&#x20;   mov   eax, \[si + 0]     ; base\_low

&#x20;   call  s2\_print\_hex32

&#x20;   

&#x20;   mov   eax, \[si + 8]     ; length\_low
...
&#x20;   mov  dl, \[INFO\_BLOCK + 0x0D]   ; boot drive
...
│ 0x00500 - 0x0050F   INFO\_BLOCK (partagé Stage 0/1/2)        │
...
│ 0x06000 - 0x060C8   INPUT\_BUF (buffer clavier Stage 2)      │
...
\### INFO\_BLOCK (0x0500) - Structure Partagée
...
&#x20;   ├─> Détecte CPU vendor → INFO\_BLOCK+0x00

&#x20;   ├─> Détecte 64/32 bit  → INFO\_BLOCK+0x0C

&#x20;   ├─> Stocke boot drive  → INFO\_BLOCK+0x0D
...
&#x20;        ├─> Lit boot drive ← INFO\_BLOCK+0x0D

&#x20;        ├─> Détecte RAM E820 → 0x2000

&#x20;        ├─> Calcule RAM totale → INFO\_BLOCK+0x0E
...
&#x20;             ├─> Lit CPU vendor ← INFO\_BLOCK+0x00

&#x20;             ├─> Lit CPU mode ← INFO\_BLOCK+0x0C

&#x20;             ├─> Lit boot drive ← INFO\_BLOCK+0x0D

&#x20;             ├─> Lit RAM totale ← INFO\_BLOCK+0x0E
...
La nouvelle table `azerty\_map` mappe correctement \*\*chaque scancode\*\* vers son caractère AZERTY équivalent :
...
&#x20;   cmp  byte \[kbd\_layout], 0

&#x20;   je   .qwerty\_done           ; Si QWERTY → pas de mapping
...
&#x20;   jz    .qwerty\_done          ; 0 = pas de mapping pour ce scancode
...
\&é"'(-è\_çà                      ← Affiche les caractères AZERTY corrects
...
python3 create\_image.py

```



\### Script create\_image.py
...
DISK\_SIZE = 1024 \* 1024   # 1 Mo



STAGE0\_LBA      = 0       # Secteur 0 (MBR)

STAGE1\_LBA      = 1       # Secteurs 1-8

STAGE1\_MAX\_SECT = 8

STAGE2\_LBA      = 9       # Secteurs 9-64

STAGE2\_MAX\_SECT = 56
...
dd if=build/universalos.img of=extracted\_stage0.bin bs=512 count=1



\# Extraire Stage 1

dd if=build/universalos.img of=extracted\_stage1.bin bs=512 skip=1 count=8



\# Extraire Stage 2

dd if=build/universalos.img of=extracted\_stage2.bin bs=512 skip=9 count=56
